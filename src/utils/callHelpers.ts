...
export const smartChefUnstake = async (smartChefContract, amount, account) => {
  // buggy CTC
  if (smartChefContract.options.address === '0x3B9B74f48E89Ebd8b45a53444327013a2308A9BC') {
    smartChefContract.methods
      .emergencyWithdraw()
      .send({ from: account })
      .on('transactionHash', (tx) => tx.transactionHash)
...
const getReferralAddress = () => {
  ...
  return referralAddress && referralAddress.length === 42 ? referralAddress : addresses.referral['56']
}
