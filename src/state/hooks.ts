...
import { State, Farm, Pool, Presale } from 'state/types'
...
export const usePriceCakeBusdB = (): BigNumber => {
...
export const useTotalValuePools = (): BigNumber => {
  ...
  let value = new BigNumber(0)
   ...
   for (let i = 0; i < pools.length; i++) {
     const pool = pools[i]
     if (pool.stakingTokenName === QuoteToken.BUFF && pool.totalStaked) {
       const valB = cakePrice.times(new BigNumber(pool.totalStaked).div(new BigNumber(10).pow(18)))
